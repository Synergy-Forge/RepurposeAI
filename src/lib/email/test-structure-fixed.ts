 ...
 * Run with: npx tsx src/lib/email/test-structure-fixed.ts
 */
...
async function testEmailStructure() {
  ...
  try {
    ...
    // Welcome Email
    const welcomeHtml = await render(WelcomeEmail({ data: testData }));
    ...
    const processingHtml = await render(ProcessingCompleteEmail({ data: processingData }));
    ...
    const subscriptionHtml = await render(SubscriptionActivatedEmail({ data: subscriptionData }));
